...
class DQNAgent:
    def __init__(self, model):
        ...
        self.params = {'value': 0.5, 'entropy': 0.0001, 'gamma': 0.90}
        self.model = model
        self.model.compile(
            # optimizer=ko.Adam()
    ...
    def train(self, env, batch_sz=32, updates=1000, see_each=50):
        memory = Memory(50000)
        ...
        for update in range(updates):
            ...
            next_rewards = self.model.call(mb_next_obs)
            next_reward = tf.math.reduce_max(next_rewards, axis=1)
            # next_reward = tf.math.reduce_mean(next_reward, axis=1)
            # advs = mini_batch.reward * (1 - np.array(mini_batch.done)) + self.params['gamma'] * next_reward * (1 - np.array(mini_batch.done))
            advs = (1-self.params['gamma']) + self.params['gamma'] * next_reward * (1 - np.array(mini_batch.done))
            ...
            print(losses)
            comp_rewards = self.model.call(mb_next_obs)
            diff = tf.math.subtract(comp_rewards, next_rewards)
...
class DQNModel(tf.keras.Model):
    def __init__(self, num_actions):
        ...
        self.logits = kl.Dense(num_actions, activation='relu')
